...
class Day08 {
    ...
    private val data = "/day08.in"
    private val sample = "/day08_sample.in"

    // 1 ligne dans les input:    10 signal patterns | 4 digit output values
    data class Display(val patterns: Array<CharArray>, val digits: Array<CharArray>) {
        ...
        companion object {
            fun parse(line: String): Display {
                ...
                return Display(
                    parts[0].split(" ").map { it.toCharArray().sorted().toCharArray() }.toTypedArray(),
                    parts[1].split(" ").map { it.toCharArray().sorted().toCharArray() }.toTypedArray()
                )
            }
        }
    }
